...
#         self.display_load_image("T_Display1.dat",80,40)
    ...
    def send_mail(self, delta_t, pontos_v, corpo, address):
        self.display_load_image(
            "T_Display5.dat", self.wifi_logo_x0, self.wifi_logo_y0)
        url = "http://raposa.ist.utl.pt/se/SendMail.php"
        print(url)
        csv = ""
        for n in range(len(pontos_v)):
            # Pontos em volt com 3 casas decimais
            csv += "%.4f,%.3f\n" % (delta_t*n, pontos_v[n])
        ...
        try:
            ...
            self.display_load_image(
                "T_Display3.dat", self.wifi_logo_x0, self.wifi_logo_y0)
            return

        if "OK Message" in result.text:
            print("send_mail(): OK")
            self.display_load_image(
                "T_Display4.dat", self.wifi_logo_x0, self.wifi_logo_y0)
        else:
            print("send_mail(): Failed")
            self.display_load_image(
                "T_Display3.dat", self.wifi_logo_x0, self.wifi_logo_y0)
    ...
    def display_write_grid(self, x=0, y=0, w=WIDTH, h=HEIGHT, nx=10, ny=8, center_lines=True, line_color=GREY1, border_color=GREY2):
        ...
        for n in range(nx+1):
            if n == 0 or n == nx:
                ...
                if center_lines:
                    self.display_set(border_color, int(
                        x + dx * n + 0.5), y, 1, h - 1)
                else:
                    self.display_set(line_color, int(
                        x + dx * n + 0.5), y, 1, h - 1)
            else:
        ...
        for n in range(ny+1):
            if n == 0 or n == ny:
                ...
                if center_lines:
                    self.display_set(border_color, x, int(
                        y + dy * n + 0.5), w - 1, 1)
                else:
                    self.display_set(line_color, x, int(
                        y + dy * n + 0.5), w - 1, 1)
            else:
    ...
    def display_write_ch(self, ft, ch, x=0, y=0, foreground=0xffff, background=0x0000):
        ...
        self.win.signal_message.emit(
            ('DISPLAY_WRITE_CH', image, foreground, background, x, y, w, h))
        return (h)
    ...
    def mythread(self):
        ...
        self.win = MainWindow()
        self.win.ui.lineEdit_4.setText(
            QCoreApplication.translate("MainWindow", self.code, None))
...
#                self.ui.label.setStyleSheet("border: 2px solid white;")
        ...
        self.ui.groupBox_3.setTitle(
            "Upload file %s as main.py" % os.path.basename(sys.argv[0]))
        ...
        if exists(file_name):
            ...
            try:
                response = requests.post(
                    'http://raposa.tecnico.ulisboa.pt/se/firmware.php', headers=headers, files=files)
                print("RESPONSE: ", response.text)
                if response.text == "OK":
                    self.ui.label_10.setStyleSheet('color: black')
                    self.ui.label_10.setText(
                        "OK: File %s uploaded to server" % file_name)
                else:
                    self.ui.label_10.setStyleSheet('color: red')
                    self.ui.label_10.setText(
                        "ERROR: Failed upload %s to server" % file_name)
            except:
                self.ui.label_10.setStyleSheet('color: red')
